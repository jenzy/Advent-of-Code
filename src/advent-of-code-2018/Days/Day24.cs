...
and the defending group's immunities and weaknesses. By default, an attacking group would deal damage equal
to its effective power to the defending group. However, if the defending group is immune to the attacking group's attack type,
the defending group instead takes no damage; if the defending group is weak to the attacking group's attack type,
...
if a defending group contains 10 units with 10 hit points each and receives 75 damage,
...
How many units does the immune system have left after getting the smallest boost it needs to win?
    ...
    internal class Day24 : DayBase
    {
        public override object Part1()
        {
            var state = Parse();
            state.Battle();
            return state.Groups.Sum(g => g.Units);
        }
        ...
        public override object Part2()
        {
            ...
            while(true)
            {
                var state = origState.Clone(++boost);
                if (state.Battle() == "S")
                    return state.Groups.Sum(g => g.Units);
            }
        }
        ...
        private State Parse()
        {
            ...
            var regex = new Regex(@"(?<N>\d+) units each with (?<HP>\d+) hit points (\((?<MOD>.+)\))? ?with "
                                    + @"an attack that does (?<A>\d+) \b(?<DMG>\w+)\b damage at initiative (?<I>\d+)");
            ...
            foreach (var line in Input.Split("\n"))
            {
                ...
                else if (regex.TryMatch(line, out var match))
                {
                    var g = new Group
                    {
                        Type = type,
                        Units = int.Parse(match.Get("N")),
                        HealthPerUnit = int.Parse(match.Get("HP")),
                        DmgPerUnit = int.Parse(match.Get("A")),
                        DmgType = match.Get("DMG"),
                        Initiative = int.Parse(match.Get("I")),
                    };

                    string mods = match.Get("MOD");
                    if (!string.IsNullOrWhiteSpace(mods))
                    {
                        foreach (var mod in mods.Split("; "))
                        {
                            if (mod.StartsWith("weak to "))
                                g.WeakTo = mod.Substring("weak to ".Length).Split(", ");
                            else if (mod.StartsWith("immune to "))
                                g.ImmuneTo = mod.Substring("immune to ".Length).Split(", ");
                        }
                    }
        ...
        private class State
        {
            public State(IEnumerable<Group> groups) => Groups = groups.ToHashSet();

            public ISet<Group> Groups { get; }

            private static int GetDamage(Group attacking, Group defending)
            {
                if (defending.ImmuneTo.Contains(attacking.DmgType))
                    return 0;
                return defending.WeakTo.Contains(attacking.DmgType) ? 2 * attacking.EffectivePower : attacking.EffectivePower;
            }

            private int Attack(Group attacking, Group defending)
            {
                int deaths = GetDamage(attacking, defending) / defending.HealthPerUnit;
                defending.Units -= deaths;
                if (defending.Units <= 0)
                    Groups.Remove(defending);
                return deaths;
            }

            public string Battle()
            {
                while (Groups.Select(g => g.Type).Distinct().Count() > 1)
                {
                    var unargeted = this.Groups.ToHashSet();
                    foreach (var attacker in Groups.OrderByDescending(g => g.EffectivePower).ThenByDescending(g => g.Initiative))
                    {
                        attacker.Target = unargeted.Where(t => t.Type != attacker.Type)
                                                   .Select(target => new { target, dmg = GetDamage(attacker, target) })
                                                   .Where(x => x.dmg > 0)
                        ...
                        if (attacker.Target != null)
                            unargeted.Remove(attacker.Target);
                    }

                    bool deaths = false;
                    foreach (var attacker in Groups.OrderByDescending(g => g.Initiative).Where(g => g.Units > 0 && g.Target != null))
                        deaths |= this.Attack(attacker, attacker.Target) > 0;

                    if (!deaths)
                        return null; // stalemate
                }

                return this.Groups.Select(x => x.Type).First();
            }

            public State Clone(int boost) => new State(this.Groups.Select(g => g.Clone(g.Type == "S" ? boost : 0)));
        }
        ...
        private class Group
        {
            ...
            public int HealthPerUnit { get; set; }

            public int DmgPerUnit { get; set; }
            ...
            public int EffectivePower => Units * DmgPerUnit;

            public Group Target { get; set; }

            public Group Clone(int boost) => new Group
            {
                Type = this.Type,
                Units = this.Units,
                DmgPerUnit = this.DmgPerUnit + boost,
                DmgType = this.DmgType,
                HealthPerUnit = this.HealthPerUnit,
