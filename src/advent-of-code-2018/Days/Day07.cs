...
that's what it looks like from the pictures in the instructions."
...
must be finished before others can begin (your puzzle input). Each step is designated by a single letter.
...
With 5 workers and the 60+ second step durations described above, how long will it take to complete all of the steps?
    ...
    public class Day07 : DayBase
    {
        public override object Part1()
        {
            ...
            while (noDeps.Count > 0 || graph.Count > 0)
            {
                var step = graph.Where(g => g.Value.All(p => !graph.ContainsKey(p) && !noDeps.Contains(p)))
                                .Select(g => g.Key)
                                .Concat(noDeps)
                                .Min();
        ...
        public override object Part2()
        {
            ...
            while (noDeps.Count > 0 || graph.Count > 0)
            {
                foreach (var stepDone in workers.Where(x => x.Value == time).Select(x=>x.Key).ToList())
                {
                    workers.Remove(stepDone);
                    graph.Remove(stepDone);
                    noDeps.Remove(stepDone);
                }

                if (workers.Count == maxWorkers)
                {
                    time = workers.Values.Min();
                    continue;
                }
