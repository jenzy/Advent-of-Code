...
For example:
    ...
    For grid serial number 42, the largest total square (with a total power of 119) is 12x12 an
...
What is the X,Y,size identifier of the square with the largest total power?
    ...
    public class Day11 : DayBase
    {
        ...
        private (int x, int y, int maxSize) Solve(int serial, int maxSize)
        {
            const int gridSize = 300;
            var grid = new int[gridSize+1, gridSize+1];
            for (int y = 1; y <= gridSize; y++)
                for (int x = 1; x <= gridSize; x++)
                    grid[x, y] = grid[x-1, y] + grid[x, y-1] - grid[x-1, y-1] + PowerLevel(x, y, serial);

            var maxCord = (x: 0, y: 0, size: 0, level: int.MinValue);
            for (int x = 1; x <= gridSize-2; x++)
            {
                for (int y = 1; y <= gridSize-2; y++)
                {
                    for (int size = 3; size <= Math.Min(maxSize, gridSize - Math.Max(x, y)); size++)
                    {
                        int mm = grid[x-1, y-1] + grid[x+size-1, y+size-1] - grid[x-1, y+size-1] - grid[x+size-1, y-1];
                        if (mm > maxCord.level)
                            maxCord = (x, y, size, mm);
                    }
                }
            ...
            return (maxCord.x, maxCord.y, maxCord.size);
        }
