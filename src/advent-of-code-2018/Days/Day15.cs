...
as doing so would be dishonorable. When multiple choices are equally valid,
...
does not do any prediction about where units will be later.
...
the step which is first in reading order is chosen. For example:
...
each open square is marked with its distance from the destination square;
...
they all are either in range of a target or cannot find any square in range of a target,
...
The Elf is in range of three targets: the Goblin above it (with 4 hit points), the Goblin to its right (with 2 hit points),
and the Goblin below it (also with 2 hit points). Because three targets are in range, the ones with the lowest hit points are selected:
...
what is the outcome of the combat described in your puzzle input?
    ...
    public class Day15 : DayBase
    {
        ...
        {
            var origState = Parse();
            int elfDmg = 3, result = -1;

            while (result < 0)
                result = Battle(origState.Clone(++elfDmg), breakOnElfDeath: true);
        ...
        private State Parse()
        {
            ...
            var units = new Dictionary<(int x, int y), Unit>();
            for (int y = 0; y < map.Length; y++)
            {
                for (int x = 0; x < map[y].Length; x++)
                {
                    ...
                    {
                        units.Add((x, y), new Unit(x, y, map[y][x]));
                        map[y][x] = '.';
                    }
                }
            }

            return new State { Map = map, Units = units };
        }

        private int Battle(State state, bool breakOnElfDeath)
        {
            int iRound = 0;
            while (true)
            {
                var turnUnits = state.Units.Values.OrderBy(u => u.Y).ThenBy(u => u.X).ToList();
                foreach (var unit in turnUnits.Where(u => u.Health > 0))
                {
                    var enemies = state.Enemies(unit).ToList();
                    if (!enemies.Any())
                        return iRound * state.Units.Values.Sum(u => u.Health);

                    var enemiesInRange = state.EnemiesInRange(unit).ToList();
                    if (!enemiesInRange.Any())
                    {
                        var enemyAdjacents = enemies.SelectMany(e => state.Neighbours(e.Location)).Where(n => !state.Occupied(n));
                        var path = state.FindPath(unit, enemyAdjacents.ToHashSet());
                        if (path == null)
                            continue;

                        state.MoveUnit(unit, path[1]);
                        enemiesInRange = state.EnemiesInRange(unit).ToList();
                        if (!enemiesInRange.Any())
                            continue;
                    }

                    var enemy = enemiesInRange.OrderBy(e => e.Health).ThenBy(e => e.Y).ThenBy(e => e.X).First();
                    if (state.Attack(unit, enemy) && breakOnElfDeath && enemy.Type == 'E')
                        return -1;
                }
        ...
        private class State
        {
            ...
            public IDictionary<(int x, int y), Unit> Units { get; set; }

            public int ElfDamage { get; private set; } = 3;

            public IEnumerable<Unit> EnemiesInRange(Unit unit)
            {
                foreach (var neigbourLoc in Neighbours(unit.Location))
                    if (Units.TryGetValue(neigbourLoc, out var enemy) && unit.Type != enemy.Type)
                        yield return enemy;
            }

            public IEnumerable<Unit> Enemies(Unit unit) => Units.Values.Where(u => u.Type != unit.Type);

            public bool Occupied((int x, int y) loc) => Map[loc.y][loc.x] != '.' || Units.ContainsKey(loc);

            public IEnumerable<(int x, int y)> Neighbours((int x, int y) loc)
            {
                yield return (loc.x, loc.y - 1);
                yield return (loc.x - 1, loc.y);
                yield return (loc.x + 1, loc.y);
                yield return (loc.x, loc.y + 1);
            }

            public List<(int x, int y)> FindPath(Unit unit, ISet<(int x, int y)> targets)
            {
                var visited = new HashSet<(int x, int y)> { unit.Location };
                var queue = new Queue<List<(int x, int y)>>();
                queue.Enqueue(new List<(int x, int y)> { unit.Location });

                var paths = new LinkedList<List<(int x, int y)>>();
                while (queue.Any())
                {
                    var curPath = queue.Dequeue();
                    if (targets.Contains(curPath.Last()))
                    {
                        paths.AddLast(curPath);
                    }
                    else
                    {
                        var candidates = this.Neighbours(curPath.Last()).Where(n => !this.Occupied(n) && !visited.Contains(n));
                        foreach (var c in candidates)
                        {
                            visited.Add(c);
                            queue.Enqueue(curPath.Concat(Enumerable.Repeat(c, 1)).ToList());
                        }
                    }
                }
                ...
                return paths.OrderBy(p => p.Count).ThenBy(p => p.Last().y).ThenBy(p => p.Last().x).First();
            }

            public void MoveUnit(Unit unit, (int x, int y) newLocation)
            {
                Units.Remove(unit.Location);
            ...
            {
                defender.Health -= attacker.Type == 'E' ? this.ElfDamage : 3;
                if (defender.Health <= 0)
                    Units.Remove(defender.Location);
                return defender.Health <= 0;
            }

            public State Clone(int? elfDmg) => new State
            {
                Map = this.Map,
                Units = this.Units.Values.Select(u => u.Clone()).ToDictionary(u => u.Location),
                ElfDamage = elfDmg ?? this.ElfDamage
            };
            ...
            public void Print()
            {
                for (int y = 0; y < Map.Length; y++)
                {
                    ...
                    for (int x = 0; x < Map[y].Length; x++)
                    {
                        if (Units.TryGetValue((x, y), out var unit))
                        {
                            list.AddLast(unit);
                     ...
                     foreach ( var unit in list)
                        Console.Write($"{unit.Type}({unit.Health}), ");
                    Console.WriteLine();
                }
            }
        }
        ...
        private class Unit
        {
            public Unit(int x, int y, char type) => (X, Y, Type) = (x, y, type);
