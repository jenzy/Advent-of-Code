...
This regex begins with ENWWW, which means that from your current position, all routes must begin by moving east, north, and then west three times,
in that order. After this, there is a branch. Before you consider the branch, this is what you know about the map so far,
...
How many rooms have a shortest path from your current location that pass through at least 1000 doors?
    ...
    public class Day20 : DayBase
    {
        ...
        public override object Part2() => Solve().Count(x => x >= 1000);
        ...
        private IEnumerable<int> Solve()
        {
            ...
            var cur = (x: 0, y: 0);
            var prev = cur;
            var shortestPath = new Dictionary<(int x, int y), int> { [cur] = 0 };

            foreach (char c in Input.TrimStart('^').TrimEnd('$'))
            {
                switch (c)
                {
                    case '(':
                    ...
                    default:
                        cur = Move(cur, c);
                        var newLength = shortestPath[prev] + 1;
                        if (!shortestPath.TryGetValue(cur, out int curLength) || curLength > newLength)
                            shortestPath[cur] = newLength;
                        break;
                }
            ...
            return shortestPath.Values;
        }

        private static (int x, int y) Move((int x, int y) p, char dir)
        {
            switch (dir)
            {
