...
growing these plants to make hot chocolate! As you realize this, you hear a conversation
in the distance. When you go to investigate, you discover two Elves in what appears to be
a makeshift underground kitchen/laboratory.
...
1 plus the score of their current recipe. So, after the first round,
the first Elf moves forward 1 + 3 = 4 times, while the second Elf moves forward 1 + 7 = 8 times.
If they run out of recipes, they loop back around to the beginning. After the first round,
both Elves happen to loop around until they land on the same recipe that they had in the beginning;
...
How many recipes appear on the scoreboard to the left of the score sequence in your puzzle input?
    ...
    internal class Day14 : DayBase
    {
        public override object Part1()
        {
            ...
            while (recipes.Count < skip + 10)
            {
                int combined = recipes[elf1] + recipes[elf2];
                int new1 = combined / 10, new2 = combined % 10;
                if (new1 > 0)
                    recipes.Add(new1);
                recipes.Add(new2);
            ...
            return string.Join("", recipes.Skip(skip).Take(10));
        }

        public override object Part2()
        {
            int target = int.Parse(Input), targetMod = (int)Math.Pow(10, Input.Length);
            var recipes = new List<int> { 3, 7 };
            int elf1 = 0, elf2 = 1, num = 37;
