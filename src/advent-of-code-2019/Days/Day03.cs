...
ecause the wires are on a grid, use the Manhattan distance for this measurement.
...
What is the fewest combined steps the wires must take to reach an intersection?
    ...
    public class Day03 : DayBase
    {
        public override object Part1()
        {
            ...
            var l1 = ExpandPath(data[0]);
            var l2 = ExpandPath(data[1]);
            
            return l1.Keys.Where(t => l2.ContainsKey(t)).Select(t => Math.Abs(t.x) + Math.Abs(t.y)).Min();
        }
        ...
        public override object Part2()
        {
            ...
            return l1.Keys.Where(t => l2.ContainsKey(t)).Select((t => l1[t] + l2[t])).Min();
        }

        private Dictionary<(int x, int y), int> ExpandPath(IEnumerable<string> path)
        {
            var line = new Dictionary<(int x, int y), int>();
