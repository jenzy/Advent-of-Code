...
and Callisto's x velocity changes by -1 (because 3 < 5). However, if the positions on a given axis are the same, the velocity on that axis does not change for that pair of moons.
...
How many steps does it take to reach the first state that exactly matches a previous state?
    ...
    public class Day12 : DayBase
    {
        public override object Part1()
        {
            ...
            return moons.Select(m => m.Position.Select(Math.Abs).Sum() * m.Velocity.Select(Math.Abs).Sum()).Sum();
        }
        ...
        private class Moon
        {
            ...
            public int[] Velocity { get; set; } = new int[3];

            public string GetState(int i) => Position[i] + "," + Velocity[i];
        }
    }
}
