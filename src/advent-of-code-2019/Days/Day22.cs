...
At least you're drifting in the right direction. You decide to practice a new card shuffle you've been working on.

Digging through the ship's storage, you find a deck of space cards!
Just like any deck of space cards, there are 10007 cards in the deck numbered 0 through 10006.
...
You've also been getting pretty good at a version of this technique where N is negative!
In that case, cut (the absolute value of) N cards from the bottom of the deck onto the top. For example, to cut -4:
...
the card to its right ends up just below the top card, and so on, until the rightmost card ends up at the bottom of the deck.
...
After shuffling your new, giant, factory order deck that many times, what number is on the card that ends up in position 2020?
    ...
    {
        public override object Part1() => ShuffleCard(2019, 10007, Parse(Input));

        public override object Part2()
        {
            const long nCards = 119315717514047;
            const long numRepeats = 101741582076661;
            var shuffles = Parse(Input).ToList();

            long i0 = ShuffleCard(0, nCards, shuffles), i1 = ShuffleCard(1, nCards, shuffles);
            var a1 = (i1 - i0 + nCards) % nCards;

            var a = ModInverse(a1, nCards);
            var b = Mod(-a * i0, nCards);
            var ar = BigInteger.ModPow(a, numRepeats, nCards);

            return Mod((ar * 2020) + ((ar - 1) * ModInverse(a - 1, nCards) * b), nCards);
        }

        private static IEnumerable<(string verb, int num)> Parse(string input)
        {
            return input.Split('\n').Select(line =>
            {
                var spl = line.Split();
                return (verb: spl[0], num: int.TryParse(spl.Last(), out var num) ? num : 0);
            });
        }
        ...
        public static void Test()
        {
            ...
            Assert.Equal(2604L, day.Part1());
            Assert.Equal((BigInteger)79608410258462, day.Part2());
        }

        private long ShuffleCard(long iCard, long n, IEnumerable<(string verb, int num)> shuffles)
        {
            foreach (var shuffle in shuffles)
            {
                iCard = shuffle switch
                {
                    ("cut", int num) => (iCard - num) % n,
                    ("deal", int num) when num > 0 => (iCard * num) % n,
                    ("deal", _) => n - iCard - 1,
        ...
        private BigInteger Mod(BigInteger n, BigInteger x) => ((n % x) + x) % x;
        ...
        private static BigInteger ModInverse(BigInteger value, BigInteger modulo)
        {
            ...
            var (gcd, lf, _) = Egcd(value, modulo);
            if (gcd != 1)
                throw new ArgumentException("Invalid modulo", nameof(modulo));
            return (lf + modulo) % modulo;

            (BigInteger gcd, BigInteger lf, BigInteger rf) Egcd(BigInteger left, BigInteger right)
            {
                BigInteger leftFactor = 0, rightFactor = 1, u = 1, v = 0, gcd = 0;
                ...
                return (gcd, leftFactor, rightFactor);
            }
        }
    }
}