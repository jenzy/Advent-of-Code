...
instead, they're encoded in a special Space Image Format. None of the Elves seem to remember why this is the case. They send you the instructions to decode it.
...
What message is produced after decoding your image?
    ...
    public class Day08 : DayBase
    {
        ...
        {
            var layer = Chunks(new Memory<byte>(Parse(Input).ToArray())).MinBy(x => x.Count(y => y == 0));
            return layer.Count(x => x == 1) * layer.Count(x => x == 2);
        }

        public override object Part2()
        {
            var chunks = Chunks(new Memory<byte>(Parse(Input).ToArray())).ToArray();
            var image = chunks.Last();
        ...
        private IEnumerable<byte[]> Chunks(Memory<byte> data, int len = 150)
        {
            for (int i = 0; i < data.Length; i += len)
                yield return data.Slice(i, len).ToArray();
        }
