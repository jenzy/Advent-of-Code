...
You can send a droid out to investigate,
but the tumbling is causing enough artificial gravity that one wrong step could send
...
it will use an output instruction to indicate the amount of damage to the hull as a single giant integer outside the normal ASCII range.
...
Successfully survey the rest of the hull by ending your program with RUN. What amount of hull damage does the springdroid now report?
    ...
    public class Day21 : DayBase
    {
        ...
        private long RunSpringScript(params string[] script)
        {
            var tmp = (string.Join("\n", script) + "\n");
            var ic = new Intcode(Parse(Input), tmp.Select(x => (long)x));
            ic.Run();

            while (ic.Output.Peek() <= 127)
                Console.Write((char)ic.Output.Dequeue());

            return ic.Output.Dequeue();
        }
        ...
        public static void Test()
        {
            ...
            Assert.Equal(19357335L, day.Part1());
            Assert.Equal(1140147758L, day.Part2());
        }
    }
}